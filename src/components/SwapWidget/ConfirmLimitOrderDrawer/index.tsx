...
import { KLC, Token, Trade, TradeType } from '@kalycoinproject/sdk';
...
const ConfirmLimitOrderDrawer: React.FC<Props> = (props) => {
  ...
  const getInputCurrency = () => {
    if (inputCurrency1 && inputCurrency1?.symbol === KLC[chainId].symbol) {
      return KLC[chainId];
  ...
  const getOutputCurrency = () => {
    if (outputCurrency1 && outputCurrency1?.symbol === KLC[chainId].symbol) {
      return KLC[chainId];
