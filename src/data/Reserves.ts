...
import { ChainId, Currency, Pair, TokenAmount } from '@kalycoinproject/sdk';
import { useMemo } from 'react';
import { useKalyswapWeb3 } from '../hooks';

import { useMultipleContractSingleData } from '../state/pmulticall/hooks';
import { wrappedCurrency } from '../utils/wrappedCurrency';

const PAIR_INTERFACE = new Interface(IKalyswapPair.abi);
    ...
    () =>
      tokens.map(([tokenA, tokenB]) => {
        return tokenA && tokenB && !tokenA.equals(tokenB)
          ? Pair.getAddress(tokenA, tokenB, chainId ? chainId : ChainId.KALYCHAIN)
          : undefined;
      }),
    [tokens, chainId],
  );
