...
/**
 * Returns a map of the given addresses to their eventually consistent KLC balances.
 */
...
export function useCurrencyBalances(
  ...
  const containsETH: boolean = useMemo(
    () => currencies?.some((currency) => chainId && currency === KLC[chainId]) ?? false,
    [chainId, currencies],
  );
  ...
  return useMemo(
    () =>
      currencies?.map((currency) => {
        ...
        if (currency === KLC[chainId]) return ethBalance[account];
        return undefined;
      }) ?? [],
