...
const currencyKey = (currency: Currency, chainId: ChainId): string => {
  return currency instanceof Token ? currency.address : currency === KLC[chainId] ? 'KLC' : '';
};
...
const SelectTokenDrawer: React.FC<Props> = (props) => {
  ...
  const currencies = useMemo(() => {
    if (searchQuery === '') {
      ...
      const _tokens = filteredSortedTokens.filter((token) => token !== KLC[chainId]);
      return [KLC[chainId], ..._tokens];
    }
    return filteredSortedTokens;
  }, [filteredSortedTokens, chainId]);
  //const currencies = useMemo(() => [Currency.KLC, ...filteredSortedTokens], [filteredSortedTokens]);
