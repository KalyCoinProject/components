...
export function useCurrency(currencyId: string | undefined): Currency | null | undefined {
  const chainId = useChainId();
  const isKLC = currencyId?.toUpperCase() === 'KLC';
  const token = useToken(isKLC ? undefined : currencyId);
  return isKLC ? chainId && KLC[chainId] : token;
}
...
export function useCoinGeckoTokenPrice(coin: Token) {
  ...
  useEffect(() => {
    const getCoinPriceData = async () => {
      try {
        const chain = coin.chainId === 3889 ? CHAINS[ChainId.KALYCHAIN] : CHAINS[coin.chainId];
