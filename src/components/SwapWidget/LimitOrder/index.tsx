...
import { KLC, JSBI, Token, TokenAmount, Trade } from '@kalycoinproject/sdk';
...
const LimitOrder: React.FC<Props> = ({ swapType, setSwapType, isLimitOrderVisible }) => {
  ...
  const getInputCurrency = () => {
    if (gelatoInputCurrency && gelatoInputCurrency?.symbol === KLC[chainId].symbol) {
      return KLC[chainId];
    } else if (inputTokenInfo && inputTokenInfo?.symbol === KLC[chainId].symbol) {
      return KLC[chainId];
  ...
  const getOutputCurrency = () => {
    if (gelatoOutputCurrency && gelatoOutputCurrency?.symbol === KLC[chainId].symbol) {
      return KLC[chainId];
    } else if (outputTokenInfo && outputTokenInfo?.symbol === KLC[chainId].symbol) {
      return KLC[chainId];
  ...
  const onCurrencySelect = useCallback(
    (currency) => {
      ...
      if (currency?.symbol === KLC[chainId].symbol) {
