...
const WalletModal: React.FC<WalletModalProps> = ({
  ...
  function addKALYCHAINNetwork() {
    injected.getProvider().then((provider) => {
      provider
        ?.request({
          method: 'wallet_addEthereumChain',
          params: [KALYCHAIN_CHAIN_PARAMS],
        })
        .then(() => {
          onWalletConnect();
        })
      ...
      return (
        <ContentWrapper>
          {web3Error instanceof UnsupportedChainIdError ? (
            <>
              <h5>Please connect to the appropriate KALYCHAIN network.</h5>
              {isMetamaskOrCbWallet && (
                <Button variant="primary" onClick={addKALYCHAINNetwork}>
                  Switch to KALYCHAIN Chain
                </Button>
              )}
            </>
          ) : (
