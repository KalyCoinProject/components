  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === KLC[chainId] ? 'KLC' : '',
        }),
      );
    },
    [dispatch],
...
const BAD_RECIPIENT_ADDRESSES: string[] = [
  FACTORY_ADDRESS[ChainId.KALYCHAIN], // v2 factory
  ROUTER_ADDRESS[ChainId.KALYCHAIN], // v2 router 02
];
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'KLC') return 'KLC';
    if (valid === false) return 'KLC';
  }
  //return 'KLC' ?? '';
  return '';
}
...
export function useGelatoLimitOrderDetail(order: Order) {
  ...
  const inputCurrency = order.inputToken === NATIVE && chainId ? 'KLC' : order.inputToken;
  const outputCurrency = order.outputToken === NATIVE && chainId ? 'KLC' : order.outputToken;
